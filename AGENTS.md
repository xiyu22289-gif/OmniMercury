...
3. **M3**：内容清洗流水线、HTML转MD、纯净阅读模式 ✅ **已完成**

4. **M4**：LLM通用接入、密钥管理、流式逐字渲染能力 ✅ **已完成**

5. **M5**：标签系统（手动打标 + AI 标签推荐 + 按标签筛选） ✅ **已完成**

6. **M6**：笔记批注（富文本编辑、自动保存、OPML 导出） ✅ **已完成**

7. **M7**：大模型用量统计（按模型/操作分维度统计面板） ✅ **已完成**

8. **M8**：界面多语言（中/英/繁三语 i18next 切换） ✅ **已完成**
...
| **M2 (订阅源管理与文章列表)** | ✅ **已完成** | RSS/Atom 解析、订阅源 CRUD、文章列表虚拟滚动、已读/未读状态、OPML 导入导出 |
| **M3 (内容清洗流水线)** | ✅ **已完成** | readability + jsdom + turndown 内容清洗流水线，Reader/Original 双模式切换，字体/主题定制，系统主题自动跟随 |
| **M4 (LLM接入与AI摘要/翻译)** | ✅ **已完成** | LLM Provider 抽象层、多服务商预设（DeepSeek/ChatECNU/Kimi/OpenAI）、摘要（三级详细度）/段落翻译/选择翻译/选择摘要、流式 SSE 推送、Token 用量估算与记录 |
| **M5 (标签系统)** | ✅ **已完成** | 手动打标、AI 标签推荐、按标签筛选文章、标签 CRUD |
| **M6 (笔记与文摘导出)** | ✅ **已完成** | 富文本笔记编辑器（contentEditable）、自动保存、字体/字号自定义、OPML 导出笔记 |
| **M7 (大模型用量统计)** | ✅ **已完成** | Token 消耗按模型/操作分维度统计，可视化面板集成于 LLM 设置 |
| **M8 (界面多语言)** | ✅ **已完成** | 中/英/繁三语 i18next，语言自动检测与持久化 |

**打包与发布**：
- ✅ Windows 安装包已生成
- ✅ macOS 安装包已生成（GitHub Actions）
...
| **M3** | 内容提取、清洗和 Markdown 转换，阅读模式 | ✅ 已完成 |
| **M4** | LLM 接入，文章摘要和翻译功能 | ✅ 已完成 |
| **M5** | 标签系统（手动打标 + AI推荐） | ✅ 已完成 |
| **M6** | 笔记与文摘导出 | ✅ 已完成 |
| **M7** | 大模型用量统计 | ✅ 已完成 |
| **M8** | 界面多语言 | ✅ 已完成 |
