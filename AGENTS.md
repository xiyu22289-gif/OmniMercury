...
- **主进程**：`src/main`（包含 index\.ts, preload\.ts, ipc\-handlers/）
...
- **主进程入口**：`src/main/index.ts`（注册IPC、初始化数据库、创建窗口）

- **预加载脚本**：`src/main/preload.ts`（`window.electron` API 白名单暴露）

- **渲染进程入口**：`src/renderer/main.tsx`（React挂载、全局样式注入）

- **全局状态**：`src/renderer/store/useAppStore.ts`（Zustand全局状态管理）
...
1. **M1**：项目脚手架、三进程配置、基础布局与路由

2. **M2**：RSS订阅源解析、新增、列表展示、Zustand状态管理

3. **M3**：内容清洗流水线、HTML转MD、纯净阅读模式

4. **M4**：LLM通用接入、密钥管理、流式逐字渲染能力

5. **M5\-M8**：标签分类、笔记批注、文件导出、Token统计、多语言国际化
...
## 11\. 未来规划 (Roadmap)
*   **M1**: 完成项目脚手架搭建，实现三栏布局和基本路由。
*   **M2**: 实现 RSS 订阅源的添加、解析、列表展示和状态管理。
*   **M3**: 完成内容提取、清洗和 Markdown 转换，实现阅读模式。
*   **M4**: 接入 LLM，实现文章摘要和翻译功能。
*   **M5-M8**: 依次实现标签、笔记导出、用量统计、多语言等选做功能。
