...
**当前进度**：M1 ✅ 已完成 | M2 ✅ 已完成 | M3 🔄 进行中 | M4 ⬜ 待开始
...
- **Task Detail**:
    ...
    4. **新增**：深色/浅色模式跟随系统主题（`window.matchMedia('(prefers-color-scheme: dark)')`）。
- **Affected Files**:
    - `src/renderer/components/ReaderView.tsx`
...
## Phase 4: LLM 通用接入与 AI 摘要/翻译 (M4) ⬜ 待开始（下周一前完成）
**对应里程碑**: M4  
...
- **Task Detail**:
    ...
    4. 将生成的摘要存入数据库，与文章关联，避免重复调用浪费 Token。
- **Affected Files**:
    - `src/renderer/components/ReaderView.tsx`
    - `src/main/llmService.ts`
    - `src/main/db.ts`
...
- **Task Detail**:
    ...
    3. 实现原文与译文的对照显示模式。
    4. 将翻译结果存入数据库。
    5. **新增**：翻译过程中 UI 应即时反馈，先展示加载状态或占位框。
- **Affected Files**:
    - `src/renderer/components/ReaderView.tsx`
    - `src/main/llmService.ts`
    - `src/main/db.ts`
...
## Phase 5: 选做功能 (M5-M8) ⬜ 待开始

以下为选做加分功能，有时间可推进，不做硬性要求。

| 里程碑 | 内容 | 状态 |
|:---|:---|:---|
| M5 | 标签系统（手动打标 + AI推荐 + 按标签筛选） | ⬜ 待开始 |
| M6 | 笔记与文摘导出（单篇/多篇） | ⬜ 待开始 |
| M7 | 大模型用量统计（按服务商/模型/功能） | ⬜ 待开始 |
| M8 | 界面多语言（中英双语切换） | ⬜ 待开始 |
