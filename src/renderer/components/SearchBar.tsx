...
export default function SearchBar() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    console.log('[SearchBar] 回车搜索:', query)
    try {
      const response = await window.api.searchArticles(query, undefined, undefined, 50)
      console.log('[SearchBar] 结果:', response.payload.error, '数量:', response.payload.articles?.length)
      if (response.payload.error === 0) {
        setSearchResults(response.payload.articles || [])
      }
    } catch (err) {
      console.error('[SearchBar] 搜索失败:', err)
    }
  }, [setSearchResults])
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Escape') { setIsOpen(false); handleClear() }
  }
  ...
  return (
    <div className="relative" ref={containerRef}>
      <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-700 rounded px-2 py-0.5">
        ...
        />
        {searchQuery && (
          <button onClick={handleClear} className="p-0.5 rounded hover:bg-gray-200 dark:hover:bg-gray-600">
            <X size={12} />
          </button>
        )}
      </div>
