...
import { getLlmConfig, getApiKeyForModel, getEffectiveConfig, getFunctionConfig, type LlmConfig } from './configService'
import { getDb, articles as articlesTable, insertTokenUsage } from './db'
import { eq } from 'drizzle-orm'
import type { LlmStreamChunk, LlmStreamDone, LlmStreamError, SummarizeRequest, TranslateRequest, SelectiveTranslateRequest, SelectiveSummarizeRequest, LlmFunctionType, LlmModelItem } from '../shared/types'
...
export async function translateParagraphs(request: TranslateRequest, callback: StreamCallback): Promise<void> {
  ...
  if (!effective.apiKey) { callback({ type: 'translate', articleId, message: '未配置 API Key' }); return }
  const model = effective.model
  const temp = getTemperature(model)

  let client: OpenAI
  try { client = createClientFromEffectiveConfig('fullTranslate') } catch (err) { callback({ type: 'translate', articleId, message: String(err) }); return }
